...
import math
from ladder import StraightLadder, Node
...
class LadderDrawer:
    def __init__(self, ladder: StraightLadder, padding: Tuple[float] = (0.1, 0.1)) -> None:
    ...
    def get_node_alignment(self, padding: Tuple[float]) -> None:
        ...
        for i in range(self.line_count):
            ...
            for idx, node in enumerate(node_list):
                self.node_position[node.node_id] = (self.x_start + i * self.x_step, self.y_end - idx * y_step)
    ...
    def draw_ghost_leg(self, check_point = False):
        ...
        for i in range(self.line_count):
            ...
            for node in node_list:
                ...
                if node.is_endpoint:
                    if check_point:
                        circle = Circle(xy=(nx, ny), radius=0.02, zorder=5, color="coral")
                        ax.add_patch(circle)
                    continue
                ...
                if check_point:
                    circle = Circle(xy=(nx, ny), radius=0.02, zorder=5, color="cadetblue")
                    ax.add_patch(circle)
    ...
    def draw_ghost_leg_ellipse(self, check_point = False, shift=False):
        ...
        for i in range(self.line_count):
            x = self.x_start + i * self.x_step
            ellipse = Ellipse(xy=(x, (self.y_end + self.y_start) / 2), width=0.1, height= self.y_start - self.y_end, 
                        edgecolor='slategray', fc='None', lw=1)
